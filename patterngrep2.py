...
class BurpExtender(IBurpExtender, ITab, IHttpListener):
    def registerExtenderCallbacks(self, callbacks):
        ...
        self._request_header_label.setFont(self._request_header_label.getFont().deriveFont(Font.BOLD))
        self._response_header_label.setFont(self._response_header_label.getFont().deriveFont(Font.BOLD))
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        if not messageIsRequest and self._monitoring:
            ...
            if any(re.search(pattern, response_body) for pattern in self._patterns):
    ...
    def toggle_monitoring(self, event):
        if self._monitoring:
            ...
            # Start monitoring
            self._patterns = [pattern.strip() for pattern in self._pattern_field.getText().split(",")]
            if self._patterns:
